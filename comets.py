...
class Comet(pygame.sprite.Sprite):
    def __init__(self, pos, size):
        ...
        self.image = pygame.image.load(
            os.path.join("assets/comets", "comet_master.png")
        ).convert_alpha()
        self.random_angle = random.choice(
            [0, 90, 180, 270]
        )  # items of list are random angles in 90 degree steps
        self.image = pygame.transform.rotate(
            pygame.transform.scale(self.image, (size, size)), angle=self.random_angle
        )
